...
fs.writeFileSync(path.resolve(__dirname.slice(0, -4), 'input.txt'), 'This is secret. Message about "_" symbol!',  ()=>{})
...
describe('stdout should of success', () => {
    test('should node my_ciphering_cli -c "C1-C1-R0-A" -i "./input.txt"', () => {
        expect(execFileSync('node', `my_ciphering_cli -c C1-C1-R0-A -i input.txt`.split(' '),  {encoding: "utf8"})).toBe('Myxn xn nbdobm. Tbnnfzb ferlm "_" nhteru!')
    })
})
